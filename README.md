...
When reboot is enabled, the app sends the reboot command in the background, logs that it is waiting, waits 5 minutes, reconnects over SSH, and then runs local curl health checks on the server. If any configured health check returns HTTP 200, the service is marked up and only that working health check is shown.
